        ...
        while True:
            ...
            for r in readable:
                if r is self.server:  # 每个r就是一个socket
                    ...
                    # conn_info_dict[0]
                    self.conn_info_dict[conn]['q_recv_data'] = queue.Queue()
                    # 0 代表首次接收，即接受命令, 1 表示接收数据， 2 表示发送数据
                    self.conn_info_dict[conn]['first_recv'] = 0
                    ...
                    if self.conn_info_dict[r]['first_recv'] == 0:
                        ...
                        if len(cmd_dict_json) == 0:
                            ...
                            try:
                                ...
                                # 将命令接收进去，为conn_info_dict[2]
                                self.conn_info_dict[r]['recv_cmd'] = cmd_dict
                                ...
                                if cmd_dict['func'] == 'push':      # 接收文件命令
                                    ...
                                    if self.conn_info_dict[r]['recv_cmd']['file_size'] != 0:
                                        # 1 表示接下来要接收数据
                                        self.conn_info_dict[r]['first_recv'] = 1
                                        self.conn_info_dict[r]['residue_recv_size'] = cmd_dict['file_size']

                                    r.send(json.dumps(info_dict).encode(
                                        'utf-8'))  # 开始接收文件
                                ...
                                if cmd_dict['func'] == 'pull':      # 发送文件命令
                                    ...
                                    if os.path.isfile(file_name):
                                        ...
                                        self.conn_info_dict[r]['residue_send_size'] = \
                                            os.stat(
                                                self.conn_info_dict[r]['send_cmd']['file_name']).st_size
            ...
            for w in writeable:     # 发送文件
                ...
                if self.conn_info_dict[w]['residue_send_size'] == self.conn_info_dict[w]['send_cmd']['file_size']:
                    ...
                    self.conn_info_dict[w]['file_obj'] = open(
                        self.conn_info_dict[w]['send_cmd']['file_name'], 'rb')
