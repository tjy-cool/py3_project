...
class MD5_Hash_Frame(wx.Frame):
    def __init__(self, parent):
        ...
        super(MD5_Hash_Frame, self).__init__(parent, -1,
                                             "MD5-Hash tools V1.0  --by tengjuyuan ", size=(600, 420))
        panel = wx.Panel(self)
    ...
    def setupMenuBar(self, parent):
        ...
        self.find_button = wx.Button(
            parent, -1, u"浏览(B)...", (16, 280), (80, 30))    # size = (70,25),
        self.clear_button = wx.Button(
            parent, -1, u"清除(L)", (16 + 96, 280), (80, 30))
        self.copy_button = wx.Button(
            parent, -1, u"复制(C)", (16 + 96 * 2, 280), (80, 30))
        self.save_button = wx.Button(
            parent, -1, u"保存(S)", (16 + 96 * 3, 280), (80, 30))
        self.stop_button = wx.Button(
            parent, -1, u"停止(T)", (16 + 96 * 4, 280), (80, 30))
        # 创建选择盒子
        ...
        self.static1 = wx.StaticText(
            parent, -1, '文件', pos=(20, 320), size=(50, 25))
        self.static2 = wx.StaticText(
            parent, -1, '总计', pos=(20, 350), size=(50, 25))
    ...
    def find_file(self, event):
        ...
        if dlg.ShowModal() == wx.ID_OK:
            ...
            for index, path in enumerate(dlg.GetPaths()):    # 依次循环各个文件
                ...
                info += "大小：" + str(file_info[0]) + "字节\n"
                if self.time_box.GetValue():
                    info += "修改时间：" + file_info[1] + "\n"
    ...
    def gethash_info(self, path, index):
        '''获得文件的MD5，SHA1校验值'''
        size = os.stat(path).st_size
        with open(path, 'rb') as f:
            ...
            for i in f:
                md5.update(i)
                sha1.update(i)
                crc32 = binascii.crc32(i, crc32)
                self.sigle_gauge.SetValue(f.tell() / size * 100)
                self.totle_gauge.SetValue((f.tell() / size + index) * 100)

            return (md5.hexdigest().swapcase(),
                    sha1.hexdigest().swapcase(),
                    '%8X' % crc32)
